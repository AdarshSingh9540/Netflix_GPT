    ...
    return (
        <div className='pt-[40%] md:pt-[8%] flex justify-center'>
            <form onSubmit={(e) => e.preventDefault()} action='' className='w-[98%] md:w-1/2 bg-black grid grid-cols-12 mx-8 p-2 mt-8'>
                <input ref={searchText} className='p-4 md:p-4  md:m-4 w-[94%] col-span-10' type='text' placeholder={lang[langKey].gptSearchPlaceholder} />
                <button className='w-full py-0 md:py-3  md:px-4 bg-red-800 text-white rounded-lg col-span-2  md:m-4' onClick={handleGPTSearchClick}>{lang[langKey].search}</button>
            </form>
        </div>
    );
};
